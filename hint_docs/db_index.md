...
- [3 インデックスのメリット・デメリット](#3-インデックスのいいところ悪いところ)  
- [4 どんなときに使うの？](#4-どんなときに使うの)  
- [5 インデックスの作り方・調べ方（MySQL）](#5-インデックスの作り方調べ方mysql)  
- [6 どこにインデックスを貼ればいいか考えるヒント](#6-どこにインデックスを貼ればいいか考えるヒント)

---

### 1 インデックスってなに？

インデックスは、**データベースの「検索を速くするためのしくみ」**です。

イメージで言うと、本の「さくいん（索引）」のようなものです。
...
```
このとき、`email` にインデックスがあると、

- データベースは全件を調べなくても  
- インデックスだけを見て、すぐに結果を出せます

つまり、**インデックスは「検索の近道」**のようなものです。

---

### 3 インデックスのいいところ・悪いところ
...
- `user_id` だけを条件にしたクエリ：**OK ✅**  
- `status` だけを条件にしたクエリ：**効果なし ❌**  
- `user_id` → `status` の順で使っているクエリ：**OK ✅**  
- 順番が逆（`status` → `user_id`）：**効果が出ないことがある ❌**