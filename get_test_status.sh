...
while true; do
    RESPONSE=$(curl -s -G "https://intermediate.hutc2511.dabaas.net/get_status" --data-urlencode "jobId=$JOB_ID")
    ...
    case "$STATUS" in
        "waiting")
            ...
            printf '%s\n' 'curl -N --http1.1 -H "Accept: text/event-stream" "https://intermediate.hutc2511.dabaas.net/waiting-stream"'
            ;;
        "running")
            ...
            ;;
        "finished")
            RESPONSE_SCORE=$(curl -s -G "https://intermediate.hutc2511.dabaas.net/score/$JOB_ID")
